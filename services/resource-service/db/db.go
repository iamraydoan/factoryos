...
// for the FactoryOS Resource Service (ISA-95 Equipment Hierarchy).
package db
...
// NewDB creates and tests a connection pool to PostgreSQL using validated configuration parameters.
func NewDB(ctx context.Context, cfg config.DatabaseConfig) (*DB, error) {
	...
	// Apply configuration parameters directly.
	...
	log.Printf("[PostgreSQL] Successfully connected to pool (MaxConns: %d, MinConns: %d).",
...
func (d *DB) RunMigrations(ctx context.Context) error {
	log.Println("[PostgreSQL] Running database migrations via goose...")
